...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/youtube/search", async (req: Request, res: Response) => {
    try {
      const { query, type = "video", maxResults = 20 } = req.query;
      ...
      if (type === 'video' || type === 'all') {
        // Search for videos
        const videoResponse = await axios.get(`${YOUTUBE_API_BASE}/search`, {
          params: {
            part: "snippet",
            q: searchQuery,
            type: "video",
      ...
      if (type === 'playlist' || type === 'all') {
        ...
        const playlistResponse = await axios.get(`${YOUTUBE_API_BASE}/search`, {
          params: {
            part: "snippet",
            q: query,
            type: "playlist",
        ...
        if (playlistResponse.data.items && playlistResponse.data.items.length > 0) {
          // Get playlist IDs for detailed info
          const playlistIds = playlistResponse.data.items.map((item: any) => item.id.playlistId).join(",");
          ...
          if (playlistDetailsResponse.data.items) {
            const playlists = playlistDetailsResponse.data.items.map((playlist: any) => ({
              id: playlist.id,
              type: "playlist",
